defmodule Collation.HanTest do
  use ExUnit.Case

  setup_all do
    Collation.ensure_loaded()
    Collation.Han.ensure_loaded()
    :ok
  end
  ...
  describe "Han character ordering" do
    test "CJK Unified Ideographs are recognized as unified ideographs" do
      ...
      assert Collation.ImplicitWeights.unified_ideograph?(0x4E00)
      # 龥 (U+9FA5) - last common CJK character
      assert Collation.ImplicitWeights.unified_ideograph?(0x9FA5)
    end

    test "CJK characters get implicit weights" do
      # Characters not in allkeys should get implicit weights
      key_a = Collation.sort_key("一")
      key_b = Collation.sort_key("二")
  ...
  describe "block_index/1" do
    test "core CJK unified block" do
      assert Collation.Han.block_index(0x4E00) == 0
      assert Collation.Han.block_index(0x9FFF) == 0
    end

    test "extension A" do
      assert Collation.Han.block_index(0x3400) == 1
    end

    test "extension B" do
      assert Collation.Han.block_index(0x20000) == 2
    end
  end
end
